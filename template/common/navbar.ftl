            ...
            <div class="container<#if settings.sidebar_column?? && settings.sidebar_column!='all'> two-column</#if>">
                ...
                <a class="navbar-item logo-title" href="${context!}">
                    <#if blog_logo?? && blog_logo!=''>
                        ...
                        <img class="logo-img-dark" src="${settings.night_logo!blog_logo!}" alt="${blog_title!}"
                             height="28">
                    <#else>
                        ${blog_title!}
                    </#if>
                                        ...
                                        <div class="item-dropdown-link">
                                            <#if menu.url!='#'>
                                                <a class="item" href="${menu.url!}" target="${menu.target!}"
                                                   title="${menu.name!}"><#if menu.icon?? && menu.icon!=''><i
                                                        class="m-icon ${menu.icon}"></i></#if>${menu.name!}</a>
                                            <#else>
                                                <a class="item" style="cursor:default;" href="javascript:"
                                                   title="${menu.name!}"><#if menu.icon?? && menu.icon!=''><i
                                                        class="m-icon ${menu.icon}"></i></#if>${menu.name!}</a>
                                            </#if>
                                            <i class="fa fa-angle-down item-dropdown-link-icon"
                                               style="color:var(--main)"></i>
                                        </div>
                                                    ...
                                                    <#if child.name?default('')?starts_with('#hide') != true>
                                                        <li class="item-sub-li">
                                                            <a class="item" href="${child.url!}"
                                                               target="${child.target!}"
                                                               title="${child.name!}"><#if child.icon?? && child.icon!=''>
                                                                <i
                                                                    class="m-icon ${child.icon}"></i></#if>${child.name!}
                                                            </a>
                                                            <ol class="item-sub">
                                                                <#list child.children?sort_by('priority') as child1>
                                                                    <#if child1.name?default('')?starts_with('#hide') != true>
                                                                        <li>
                                                                            <a class="item" href="${child1.url!}"
                                                                               target="${child1.target!}"
                                                                               title="${child1.name!}"><#if child1.icon?? && child1.icon!=''>
                                                                                <i
                                                                                    class="m-icon ${child1.icon}"></i></#if>${child1.name!}
                                                                            </a>
                                                                        </li>
                                                                    </#if>
                                                                </#list>
                ...
                <form data-pjax class="navbar-search" method="get" action="/search">
                    <input maxlength="16" autocomplete="off" placeholder="搜索内容..." name="keyword" value
                ...
                <#if settings.enable_meilisearch!false>
                    <i class="fa fa-search navbar-searchicon navbar-meilisearch-icon"></i>
                </#if>
            </div>
        </div>
        <div class="navbar-meilisearch">
            ...
            <div class="navbar-slideout-wrap">
                ...
                <ul class="navbar-slideout-menu">
                    ...
                    <li class="item">
                        ...
                        <@commentTag method="count"><span>累计收到 <strong>${count!"0"}</strong> 条评论
                            </span></@commentTag>
                    </li>
                </ul>
                                                                            ...
                                                                            <#list child.children?sort_by('priority') as child1>
                                                                                <#if child1.name?default('')?starts_with('#hide') != true>
                                                                                    <li>
                                                                                        <a class="link"
                                                                                           href="${child1.url!}"
                                                                                           title="${child1.name!}">${child1.name!}</a>
                                                                                    </li>
                                                                                </#if>
                                                                            </#list>
                                                                        </ul>
            ...
            <div class="search-container">
                <div class="navbar-searchout-inner">
                    <form data-pjax class="navbar-search-mobile" method="get" action="/search">
                        <input maxlength="16" autocomplete="off" placeholder="请输入关键字..." name="keyword" value
                               class="input" type="text"/>
                    ...
                    <@tagTag method="list">
                        <#if tags?size gt 0>
                            ...
                            <ul class="cloud">
                                ...
                                <#list tags as tag>
                                    <li class="item">
                                        <a style="background:${colors[nextRandom % colors?size]}"
                                           href="${tag.fullPath!}" title="${tag.name!}">${tag.name!}</a>
                                    </li>
