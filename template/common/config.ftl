...
<script type="text/javascript">
  ...
  DreamConfig["spark_input_content"] = ['${user.description?trim?replace('\n', '')?js_string}'
    <#if settings.color_character?? && settings.color_character?trim!=''>,
    '${settings.color_character?trim?js_string?replace('\\n', "','")}'</#if>];
  </#if>
  ...
  <#--  美丽搜索  -->
  let meilisearchEnable = ${((meilisearch_enable!false) && settings.enable_meilisearch)?c};
  DreamConfig["meilisearch_enable"] = meilisearchEnable;
  if (meilisearchEnable) {
  ...
  (function () {
    let isNight = DreamConfig.default_theme === 'system' ? matchMedia('(prefers-color-scheme: dark)').matches
        : localStorage.getItem('night') || DreamConfig.default_theme === 'night';
