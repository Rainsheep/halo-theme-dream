...
const commonContext = {
  ...
  initWidget() {
    ...
    // 实现将右边widget拷贝的左边
    if ($columnRight.length && $columnRightShadow.length
        && !$columnRightShadow[0].children.length) {
  ...
  initActions() {
    ...
    $bulletScreen.on('click', () => {
      let stopBulletScreen = localStorage.getItem('stop-bullet-screen')
          || false;
  ...
  widgetFixedBottom() {
    ...
    function leftSetBottom() {
      if (window.innerWidth < 769) {
        ...
        columnLeft.style.top = window.innerHeight - columnLeft.scrollHeight - 10
            + "px";
      }
    }
    ...
    function rightSetBottom() {
      if (window.innerWidth >= 1216) {
        columnRight.style.top = window.innerHeight - columnRight.scrollHeight
            - 10 + "px";
      }
    }

    if (columnLeft) {
      leftSetBottom(), $(window).resize(
          leftSetBottom), document.addEventListener("scroll", leftSetBottom);
    }
    if (columnRight) {
      rightSetBottom(), $(window).resize(
          rightSetBottom), document.addEventListener("scroll", rightSetBottom);
    }
  },
  /* 激活图片预览功能 */
  initGallery() {
    ...
    $('.main-content img:not(.not-gallery)').each(function () {
      if ($(this).parents('[data-fancybox],mew-photos').length === 0) {
        $(this).wrap(
            `<div class="gallery-item"><div data-fancybox="gallery" ${this.alt
                ? `data-caption="${this.alt}"` : ""} href="${$(this).attr('src')
    ...
    if (pathname && pathname !== "/") {
      for (let i = 0; i < $nav_menus.length; i++) {
        ...
        if (pathname.includes(cur_href) || href.includes(cur_href)) {
          activeIndex = i;
          if (pathname === cur_href || href === cur_href) {
            break;
          }
        }
      }
  ...
  /* 小屏幕搜索框 */
  searchMobile() {
    $(".navbar-searchicon:not(.navbar-meilisearch-icon)").on("click",
        function (e) {
          e.stopPropagation();
          /* 关闭侧边栏 */
  ...
  /* 美丽搜索框 */
  meiliSearch() {
    $(".navbar-meilisearch-icon").on("click", function (e) {
      e.stopPropagation();
      /* 处理开启关闭状态 */
      const $html = $("html");
      const $mask = $(".navbar-mask");
      const $above = $(".navbar-above");
      const $meili_search = $(".navbar-meilisearch");
      if (!$meili_search.hasClass("active")) {
        $html.addClass("disable-scroll");
        $mask.addClass("active");
        $above.addClass("solid");
        $meili_search.addClass("active");
        $(".ais-SearchBox-input").focus();
      }

      $(".ais-Hits-item").on("click", function (e) {
        $(".navbar-mask").click();
      });
    });
  },
  meiliSearchInit() {
    if (DreamConfig.meilisearch_enable) {
      const search = instantsearch({
        indexName: DreamConfig.meilisearch_index_name,
        searchClient: instantMeiliSearch(DreamConfig.meilisearch_url,
            DreamConfig.meilisearch_key)
      });
      search.addWidgets([
  ...
  maskClose() {
    ...
    .on("click", function (e) {
      ...
      $(".navbar-meilisearch").removeClass("active");
  ...
  initEvent() {
    ...
    function closeSelect(elem) {
      ...
      return closeSelect && closeSelect.trim() !== '' ? $elem.closest(
          closeSelect.trim()) : $elem;
    }
  ...
  /* 离屏提示 */
  offscreenTip() {
    if (Utils.isMobile() || (!DreamConfig.document_hidden_title
  ...
  /* 个人信息界面打印彩字 */
  sparkInput() {
    const sparkInputContent = DreamConfig.spark_input_content
        && DreamConfig.spark_input_content.filter(s => s.length > 0);
    if (sparkInputContent && sparkInputContent.length > 0) {
      Utils.cachedScript(
          ...
          function () {
            ...
            $('.spark-input').each(
                (index, domEle) => sparkInput(domEle, sparkInputContent));
          })
    }
  },
  /* 恋爱墙倒计时 */
  loveTime() {
    ...
    setInterval(function () {
      ...
      if (year !== 0) {
        $elem.html(
            `${year} 年 ${days} 天 ${hours} 时 ${minutes} 分 ${seconds} 秒`);
      } else {
  ...
  websiteTime() {
    ...
    if (!/^\d{4}\/\d{2}\/\d{2} \d{2}:\d{2}:\d{2}$/.test(
  ...
  /* 显示主题版本信息 */
  showThemeVersion() {
    window.logger(`%c页面加载耗时：${Math.round(
            performance.now())}ms | Theme By Dream ${DreamConfig.theme_version}`,
